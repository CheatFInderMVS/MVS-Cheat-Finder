...
# Blacklist definitions (Exactly like your original script)
...
Write-Progress -Activity "CheatFinder Scan" -Status "Exclusions Complete" -PercentComplete 10
...
Write-Progress -Activity "CheatFinder Scan" -Status "Threat Scan Complete" -PercentComplete 20
...
Write-Progress -Activity "CheatFinder Scan" -Status "Memory Integrity Complete" -PercentComplete 30
...
Write-Progress -Activity "CheatFinder Scan" -Status "Exploit Check Complete" -PercentComplete 50
...
Write-Progress -Activity "CheatFinder Scan" -Status "Prefetch Scan Complete" -PercentComplete 60
...
Write-Progress -Activity "CheatFinder Scan" -Status "Deleted Prefetches Scan Complete" -PercentComplete 70
...
Write-Progress -Activity "CheatFinder Scan" -Status "MuiCache Check Complete" -PercentComplete 80
...
Write-Progress -Activity "CheatFinder Scan" -Status "KeyAuth Check Complete" -PercentComplete 90
...
Write-Progress -Activity "CheatFinder Scan" -Status "UserAssist Check Complete" -PercentComplete 85
...
Write-Progress -Activity "CheatFinder Scan" -Status "BAM Check Complete" -PercentComplete 88
...
Write-Progress -Activity "CheatFinder Scan" -Status "Jump List Check Complete" -PercentComplete 92
    
# --- Registry Suspicious Check (Exact Original Structural Logic) ---
try {
    ...
    if ($registryOutput.Count -eq 0) {
        $registryOutput += "SUCCESS: No suspicious entries found in MuiCache hives."
    }
} catch {
...
$successCount = ($allLines | Where-Object { $_ -match '^SUCCESS' }).Count
$failureCount = ($allLines | Where-Object { $_ -match '^FAILURE' }).Count
$warningCount = ($allLines | Where-Object { $_ -match '^WARNING' }).Count
$totalChecks = $successCount + $failureCount + $warningCount

# PowerShell 5.1 safe structure block
if ($totalChecks -gt 0) {
...
if ($rate -ge 90) { $color = "Green" }
elseif ($rate -ge 70) { $color = "Yellow" }
else { $color = "Red" }
